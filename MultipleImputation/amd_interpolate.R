...
# completed patients
completed <- list.files("amd_interpolations/")
completed <- data.frame(filename = completed) %>% 
  mutate(admissionid = as.numeric(gsub(".rds","", gsub("amd_interpolations_","",filename))))

patients <- dat$patients %>%
    left_join(completed, by = "admissionid") %>%
    filter(is.na(filename))

GP_params <- readRDS("data/amd_gp_params.rds")

interpolate <- function(pt_stay_id){
    
print(paste0("starting interpolation for ", pt_stay_id))

id = filter(patients, admissionid == pt_stay_id)$id
...
sample = 100
if (N_pred > 0){
interp_mod <- cmdstan_model("models/amd_multi_interp_fixedGP.stan")
...
df <- bind_rows(df_obs, df_pred) %>%
    ...
    df <- data.frame(stay_id = pt_stay_id,
                     time = rep(x_obs, each = sample),
                     iter = rep(1:sample, times = N_obs),
                     var = rep(d_obs, each = sample),
                     value = rep(c(y_c, y_b), each = sample)) %>%
...
mean_sd <- readRDS("data/amd_oc_mean_sd.rds") %>%
  filter(var %in% c("resp_rate",
                         "spo2",
                         "GCS",
                         "fio2",
                         "heart_rate",
...
df <- left_join(df, vars, by = c("var" = "num")) %>%
  mutate(value = ifelse(var <= D_c, value*sd + mean, value))

log_group <- c(1:2,6:8)

df <- mutate(df,
                  value = ifelse(var %in% log_group,
                                 exp(value),
                                 value)) %>% 
  mutate(value = ifelse(var == 3,
                        round((inv_logit(value))*101),
                        value)) %>%
  mutate(value = ifelse(var == 5,
                        round((inv_logit(value))*12.2+2.9),
                        value)) %>%
  mutate(value = ifelse(var == 4,
                        round((inv_logit(value))*79.3+20.9),
                        value))

# get the baseline characteristics

pt_bl <- readRDS("data/oc_amd_b.rds") %>%
  filter(admissionid == pt_stay_id)

df <- filter(df, time >= 0)
    
interpolation_list <- list(admissionid = pt_stay_id,
                           pt_bl = pt_bl,
                           df = df)

saveRDS(interpolation_list,
        file = paste0("amd_interpolations/amd_interpolations_",
                      pt_stay_id,
                      ".rds"))
...
output <- foreach(i = 1:nrow(patients)) %dopar% interpolate(patients$admissionid[i])