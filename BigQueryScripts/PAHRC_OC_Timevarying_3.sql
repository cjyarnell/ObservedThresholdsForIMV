-- This query extracts the timevarying data for 48h prior to and 120h following eligibility --
-- Results are saved in PAHRC_OC_Timevarying

with pdx as (
  select *
  from `alpine-scholar-292916.PAHRC1.PAHRC_OC_Eligibility`
  where eligible = 1
)
...
where datetime_diff(ce.charttime, intime, minute) > -48*60 
   and datetime_diff(ce.charttime, intime, minute) - eligibility_time < 120*60
...
where datetime_diff(deathtime, pdx.intime, minute) - eligibility_time < 120*60
...
where datetime_diff(outtime, pdx.intime, minute) - eligibility_time < 120*60
...
select 
    pdx.stay_id
  , datetime_diff(charttime, pdx.intime, minute) - eligibility_time as time
  , "po2" as variable
  ,  bg.po2 as value
from pdx
left join `physionet-data.mimic_derived.bg` bg
on pdx.subject_id = bg.subject_id
where datetime_diff(bg.charttime, pdx.intime, minute) - eligibility_time < 120*60 
and bg.specimen = "ART."
...
where datetime_diff(o.charttime, intime, minute) > -48*60 
   and datetime_diff(o.charttime, intime, minute) - eligibility_time < 120*60 
...
where datetime_diff(pe.starttime, intime, minute) > -48*60 
   and datetime_diff(pe.starttime, intime, minute) - eligibility_time < 120*60
...
from(select 
  ...
  from pdx
   ...
   and datetime_diff(ne1.starttime, intime, minute) - eligibility_time < 120*60 
...
select 
   ...
   and datetime_diff(ne1.endtime, intime, minute) - eligibility_time < 120*60 
...
where datetime_diff(pr.charttime, intime, minute) > -48*60 
   and datetime_diff(pr.charttime, intime, minute) - eligibility_time < 120*60 
...
where datetime_diff(emar.charttime, intime, minute) > -48*60 
   and datetime_diff(emar.charttime, intime, minute) - eligibility_time < 120*60
) 
tmp left join pdx on pdx.stay_id = tmp.stay_id
where variable is not null and value is not null
and (pdx.imv_ever = 0 or tmp.time <= pdx.imv_time)
group by 1,2,3
order by 1,2