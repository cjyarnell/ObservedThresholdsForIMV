-- generates a table with a row for eligible icu stay_id and columns for each of the eligibility criteria
-- sat 97 or less, FiO2 40% or more, HFNO | NIV | NRB and not previously invasively ventilated, no trach, no goc prohibition on ett
-- results saved as PAHRC_OC_Eligibility
-- needs adjustment to make table for all icu stays (comment out the array agg lines)
...
select
  ...
  , max(imv) over (partition by stay_id order by time range unbounded preceding) as imv_ever
...
on pdx.stay_id = ce.stay_id
  ...
  -- , 223901 -- GCS components
   ...
   and ce.charttime < DATETIME_ADD(pdx.intime, INTERVAL 144 HOUR)
  group by 1,2,3,4
...
-- and  o.charttime < DATETIME_ADD(pdx.intime, INTERVAL 144 HOUR)
group by 1,2,3,4
 ) t 
...
, min(goc_eligible) as goc_eligible
, max(case when o.o2_device = 6 and datetime_diff(o.charttime, pdx.intime, minute) - eligibility_time < 120*60 then 1 else 0 end) as imv_ever
, min(case when o.o2_device = 6 then datetime_diff(o.charttime, pdx.intime, minute) - eligibility_time else null end) as imv_time
, max(case when o.o2_device = 6 and datetime_diff(o.charttime, pdx.intime, minute) - eligibility_time >= 120*60 then 1 else 0 end) as imv_after_censor
from (
select
  subject_id
, hadm_id
...
SELECT 
 ...
 , case when spo2 <= 100 and fio2 >= 40 then 1 else 0 end as sfratio_eligible
 , max(imv_ever) over (partition by tv.stay_id order by tv.charttime range between unbounded preceding and 30 following) as ineligible_imv
...
-- where datetime_diff(o.charttime, pdx.intime, minute) - eligibility_time < 120*60 
where eligible = 1
group by 3,2,1
order by 4 desc, 1