...
-- uses PAHRC_OC_Timevarying
-- results saved into PAHRC_OC_Baseline table
...
left join ( 
  select 
    ...
    , array_agg(case when variable = "po2" then value end ignore nulls order by tv.time desc)[offset(0)] as po2_bl -- lactate
  ...
  from t1
  left join `alpine-scholar-292916.PAHRC1.PAHRC_OC_Timevarying` tv
